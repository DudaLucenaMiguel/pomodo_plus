...
export function usePomodoroTimer({
  ...
  const [mode, setMode] = useState("idle"); // 'idle' | 'focus' | 'short' | 'long'
  const [cycleCount, setCycleCount] = useState(0); // focos concluídos
  const [streak, setStreak] = useState(0); // focos no conjunto atual
  ...
  const resetPhase = () => {
    const base =
      mode === "focus" ? focusSec : mode === "short" ? shortBreakSec : longBreakSec;
    restart(nextExpiry(base), false);
  };

  const skip = () => onExpire(); // força a troca de fase
